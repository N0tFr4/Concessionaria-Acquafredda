...
<body>
    ...
    <?php
        ...
        if(isset($_GET['id'])) {
            ...
            $id = $_GET['id'];
            $sql = "SELECT * FROM rappresentante WHERE id_rappresentante= $id ";
            ...
            <div class="card text-center mb-3" style="width: 20rem;">
                <div class="card-body">
                    <div class="d-flex align-items-start">
                        <a href="listarapp.php"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-left" viewBox="0 0 16 16">
                            <path fill-rule="evenodd" d="M15 8a.5.5 0 0 0-.5-.5H2.707l3.147-3.146a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L2.707 8.5H14.5A.5.5 0 0 0 15 8"/>
                        </svg> </a>
                    </div>

                    <h5 class="card-title">MODIFICA RAPPRESENTANTE</h5>
                    <p class="card-text">
    
                        <form method="post" action="modrapp.php">

                            <input type="hidden" name="id" value="<?php echo $row['id_rappresentante']; ?>">
                            
                            <div class="input-group input-group-sm mb-3">
                                <span class="input-group-text" id="inputGroup-sizing-sm">Nome</span>
                                <input type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm" value="<?php echo $row['nome_rappresentante']; ?>" name="nome" placeholder="Inserisci il nome" required>
                            </div>
                    
                            <div class="input-group input-group-sm mb-3">
                                <span class="input-group-text" id="inputGroup-sizing-sm">Cognome</span>
                                <input type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm" value="<?php echo $row['cognome_rappresentante']; ?>" name="cognome" placeholder="Inserisci il cognome" required>
                            </div>

                            <div class="input-group input-group-sm mb-3">
                                <span class="input-group-text" id="inputGroup-sizing-sm">Username</span>
                                <input type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm" value="<?php echo $row['username_rappresentante']; ?>" name="username" placeholder="Inserisci l'username" required>
                            </div>

                            <div class="input-group input-group-sm mb-3">
                                <span class="input-group-text" id="inputGroup-sizing-sm">Azienda</span>
                                <input type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm" value="<?php echo $row['azienda_rappresentante']; ?>" name="azienda" placeholder="Inserisci l'azienda" required>
                            </div>
